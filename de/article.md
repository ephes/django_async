...
Django ist eines der großen Python-Webframeworks. Im Gegensatz zu Flask verfolgt
es einen “batteries included” Ansatz, ganz ähnlich zu Ruby on Rails. D.h. die
Lernkurve ist unter Umständen am Anfang etwas steiler als bei Flask, dafür ist
es dann später einfacher, in kurzer Zeit Applikationen zu entwickeln.

Sowohl Flask als auch Django haben aber gegenüber Node.js basierten
Webframeworks den Nachteil, dass es nicht so einfach ist, unterschiedliche Tasks
gleichzeitig in Bearbeitung zu halten.

Ab Django [Version 3.1](https://docs.djangoproject.com/en/3.1/topics/async/),
die am 04.08.2020 erschienen ist, werden asynchrone Views, Middlewares und Tests
unterstützt. Async-Unterstützung für ORM, Formulare und Caching wird in
zukünftigen Versionen folgen. Wenn man die neuen async-Features nicht nutzen
möchte, muss man nichts ändern. Bestehender Code läuft ganz normal weiter.

Zwar gab es bereits seit
[Django 3.0](https://docs.djangoproject.com/en/3.0/releases/3.0/#asgi-support)
Unterstützung für [ASGI](https://asgi.readthedocs.io/en/latest/), doch konnte
man es nicht asynchron benutzen. Es sei denn für File Uploads, denn die
erreichen den View-Layer nicht, der in 3.0 noch nicht async-fähig war.

Welche Applikationen haben den größten Vorteil davon, async Views verwenden zu
können? Vor allem solche, in denen viele kleine Tasks simultan ausgeführt werden
...
builtin asyncio module. Or the Curio inspired [Trio](https://trio.readthedocs.io/en/stable/) which is using nurseries to get rid of futures which remain an
